...
def create_account():
    ...
    if request.method == 'POST':
        ...
        # Generate verification token and cache pending registration (do not save user yet)
        ...
        # Send verification email via EmailJS (code included)
        email_sent = email_service.send_verification_email_emailjs(
            data.get('email'),
            data.get('username'),
            verification_token
        )
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Always use email verification method (code-based)
            verification_token = email_service.generate_verification_token()
            expires_at = datetime.now() + timedelta(hours=1)
            # Pre-hash and store pending password change in memory
            new_hash = bcrypt.hashpw(new_password.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
            PENDING_PASSWORD_CHANGES[verification_token] = {
                'user_id': user_id,
                'new_password_hash': new_hash,
                'expires_at': expires_at
            }

            # Send code via EmailJS reset template
            ...
            email_sent = email_service.send_password_reset_email_emailjs(
                user_info['email'], user_info['username'], verification_token
            )
            if email_sent:
                return jsonify({
                    'success': True,
